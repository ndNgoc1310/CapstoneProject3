...
module rs_enc (
    ...
    assign feedback = ((state == MSG) || (state == IDLE && sop_in && valid_in)) ? (data_in ^ shift[NSYM-1]) : 10'd0;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        ...
        // FSM
            ...
            case (state)
                ...
                // Sự kiện kích hoạt: Khi sop_in (Start of Packet) và valid_in cùng lên mức cao, FSM chuyển sang trạng thái MSG.
                IDLE: begin
                    if (sop_in && valid_in) begin  // Khi bắt đầu có gói tin
                        state <= MSG;

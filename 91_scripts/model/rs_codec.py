...
def gf_poly_mul(p, q):
    ...
    res = [0] * (len(p) + len(q) - 1)   # Độ dài danh sách kết quả = len(p) + len(q) - 1
    ...
    for j in range(len(q)):         # Duyệt từng hệ số của đa thức q
        for i in range(len(p)):     # Duyệt từng hệ số của đa thức p
            ...
            res[i + j] ^= product   # Trong GF, phép cộng là phép XOR (^=)
...
def rs_encode(msg, nsym):
    ...
    res = [0] * nsym    # Đây là nơi chứa phần dư (parity) đang tính toán dở dang
    
    # 3. Duyệt qua từng symbol của tin nhắn (514 vòng lặp)
    for i in range(len(msg)):

        # Tính toán giá trị phản hồi (Feedback)
        feedback = msg[i] ^ res[nsym-1]  # Là XOR của symbol hiện tại với ngăn nhớ bậc cao nhất
        ...
        if feedback != 0:
            ...
            # Dịch chuyển và tính toán cho 29 ngăn đầu: từ res[29] xuống res[1]
            for j in range(nsym-1, 0, -1):

                # Ngăn j-1 nhận giá trị ngăn j XOR với (feedback * hệ số g)
                res[j] = old_res[j-1] ^ gf_mul(gen[j], feedback)

            # Ngăn cuối cùng nhận giá trị (feedback * hệ số g cuối)
            res[0] = gf_mul(gen[0], feedback)
    ...
    # Xuất parity từ res[29] xuống res[0]
    parity = res[::-1]
    return msg + parity
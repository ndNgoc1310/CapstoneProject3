...
if __name__ == "__main__":
    ...
    # Kiểm tra thử nghiệm
    a = 3   # Giá trị decimal bất kỳ < 1024
    b = 7  # Giá trị decimal bất kỳ < 1024
    
    sum_res = gf_add(a, b)
    mul_res = gf_mul(a, b)
    
    print(f"GF(2^10) Result for a={a}, b={b}:")
    print(f"Addition (XOR): {a} + {b} = {sum_res}")
    print(f"Multiplication: {a} * {b} = {mul_res}")
    
    # Kiểm tra tính chất: a * a^-1 = 1
    if a != 0:
        a_inv = power_table[1023 - log_table[a]]
        print(f"Inverse of {a}: {a_inv}")
        print(f"Check a * a_inv: {gf_mul(a, a_inv)}")